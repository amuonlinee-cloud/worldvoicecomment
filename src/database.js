// src/database.js
// Supabase wrapper - full featured with findOrCreateThread, comments, replies, favorites, reactions, payments, notifications, reports.
...
if (!SUPABASE_URL || !SUPABASE_KEY) {
  console.error('DIAG ERROR: Supabase env vars missing: SUPABASE_URL, SUPABASE_KEY.');
  module.exports = {};
  return;
}
...
function _buildLookupCandidates(originalLink, normalized) {
  ...
  try { candidates.push(String(originalLink).trim().replace(/[)\]\.]+$/g, '').toLowerCase().replace(/\/$/,'')); } catch (e) {}
  try { if (normalized && normalized.canonicalLink) candidates.push(String(normalized.canonicalLink).replace(/\/$/,'')); } catch (e) {}
...
async function findOrCreateThread(link, creatorTelegramId = null) {
  ...
  try {
    ...
    // try canonical_link candidates
    for (const cand of candidates) {
      try {
        const { data: found, error } = await supabase.from('threads').select('*').ilike('canonical_link', cand).limit(1).maybeSingle();
        if (error) { console.error('canonical lookup err', error && error.message); continue; }
        if (found) return found;
      } catch (e) { console.error('canonical lookup caught', e && e.message); }
    }
    ...
    if (provider && providerId) {
      ...
      } catch (e) { console.error('provider lookup err', e && e.message); }
    }
    ...
    for (const cand of candidates) {
      try {
        ...
        if (e3) { console.error('social link candidate err', e3 && e3.message); continue; }
        if (found3) return found3;
      } catch (e) { console.error('social link lookup caught', e && e.message); }
    }
    ...
    try {
      ...
      if (error) {
        console.error('findOrCreateThread insert error', error && (error.message || error));
  ...
  try {
    ...
    for (const cand of candidates) {
      try {
        ...
        if (data) return data;
      } catch (e) { console.error('getThreadByLink canonical err', e && e.message); }
    }
    if (normalized && normalized.provider && normalized.id) {
      ...
      } catch (e) { console.error('getThreadByLink provider err', e && e.message); }
    }
    for (const cand of candidates) {
      try {
        const { data: data2 } = await supabase.from('threads').select('*').ilike('social_link', cand).limit(1).maybeSingle();
        if (data2) return data2;
      } catch (e) { console.error('getThreadByLink social err', e && e.message); }
    }
    return null;
  } catch (e) { console.error('getThreadByLink err', e && e.message); return null; }
}
...
async function listFavoritesForUser(telegramId) {
  if (!telegramId) return [];
  try {
    const { data } = await supabase.from('favorites').select('id, comment_id, created_at, voice_comments( id, thread_id, telegram_file_id, first_name, username, created_at )').eq('telegram_id', telegramId).order('created_at', { ascending: false });
...
async function setAdminNotifier(text, extra={}) {
  try {
    await supabase.from('admin_logs').insert([{ message: text, meta: extra, created_at: new Date().toISOString() }]);
    return true;
  } catch (e) { console.error('setAdminNotifier err', e && e.message); return false; }
}
