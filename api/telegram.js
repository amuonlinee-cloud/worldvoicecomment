...
module.exports = async (req, res) => {
  ...
  try {
    if (!botPromise) botPromise = initBot(); // returns a Telegraf instance
    const bot = await botPromise;

    // Pass the update to telegraf to handle it
    // Do NOT pass `res` to handleUpdate (that can cause headers to be sent twice).
